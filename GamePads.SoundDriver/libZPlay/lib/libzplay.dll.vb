...
Imports Microsoft.VisualBasic.GamePads.SoundDriver.libZPlay.ComponentModel
...
Namespace libZPlay
    ...
    Module API
        ...
        ''' </summary>
        ''' <returns></returns>
        Public ReadOnly Property libzplay As String

        Sub New()
            API.libzplay = My.Application.Info.DirectoryPath & "/libzplay.dll"
            Try
                Call FileIO.FileSystem.WriteAllBytes(API.libzplay, My.Resources.libzplay, False)
            Catch ex As Exception
                ' 假设可能多个程序都使用这个模块，则可能出现文件占用的情况，忽略掉这个错误就好了
            End Try
        End Sub
