...
Public MustInherit Class FbxBinary
	...
	Private Shared ReadOnly sourceId As Byte() = {&H58, &Hab, &Ha9, &Hf0, &H6c, &Ha2, _
		&Hd8, &H3f, &H4d, &H47, &H49, &Ha3, _
		&Hb4, &Hb2, &He7, &H3d}
	Private Shared ReadOnly key As Byte() = {&He2, &H4f, &H7b, &H5f, &Hcd, &He4, _
		&Hc8, &H6d, &Hdb, &Hd8, &Hfb, &Hd7, _
		&H40, &H58, &Hc6, &H78}
	' This wasn't - it just appears at the end of every compliant file
	Private Shared ReadOnly extension As Byte() = {&Hf8, &H5a, &H8c, &H6a, &Hde, &Hf5, _
		&Hd9, &H7e, &Hec, &He9, &Hc, &He3, _
		&H75, &H8f, &H29, &Hb}
	...
	Protected Shared Function GenerateFooterCode(document As FbxNodeList) As Byte()
		...
		End If
		Try
			Return GenerateFooterCode(GetTimestampVar(timestamp, "Year"), GetTimestampVar(timestamp, "Month"), GetTimestampVar(timestamp, "Day"), GetTimestampVar(timestamp, "Hour"), GetTimestampVar(timestamp, "Minute"), GetTimestampVar(timestamp, "Second"), _
	...
	''' <returns>A 16-byte code</returns>
	Protected Shared Function GenerateFooterCode(year As Integer, month As Integer, day As Integer, hour As Integer, minute As Integer, second As Integer, _
