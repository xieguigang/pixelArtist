...
''' </remarks>
Public NotInheritable Class FbxIO
	Private Sub New()
	End Sub
	''' <summary>
	...
	Public Shared Function ReadBinary(path As String) As FbxDocument
	...
	''' <param name="path"></param>
	''' <returns>The top level document node</returns>
	Public Shared Function ReadAscii(path As String) As FbxDocument
		If path Is Nothing Then
			Throw New ArgumentNullException(nameof(path))
		End If
		Using stream = New FileStream(path, FileMode.Open)
	...
	''' <param name="path"></param>
	Public Shared Sub WriteBinary(document As FbxDocument, path As String)
		If path Is Nothing Then
			Throw New ArgumentNullException(nameof(path))
		End If
	...
	''' <param name="document">The top level document node</param>
	''' <param name="path"></param>
	Public Shared Sub WriteAscii(document As FbxDocument, path As String)
		If path Is Nothing Then
			Throw New ArgumentNullException(nameof(path))
		End If
		Using stream = New FileStream(path, FileMode.Create)
